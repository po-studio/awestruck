...
class AwestruckInfrastructure extends TerraformStack {
  constructor(scope: Construct, id: string) {
    ...
    const userDataTemplate = fs.readFileSync(
      path.join(__dirname, 'scripts/coturn-userdata.sh'),
      'utf-8'
    );

    if (!userDataTemplate.includes('${ELASTIC_IP}') || !userDataTemplate.includes('${TURN_PASSWORD}')) {
      throw new Error('User data template is missing required variables');
    }

    const userData = userDataTemplate
      .replace('${ELASTIC_IP}', coturnElasticIp.publicIp)
      .replace('${TURN_PASSWORD}', turnPassword);
